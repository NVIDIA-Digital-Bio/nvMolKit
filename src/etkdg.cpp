...
void embedMolecules(const std::vector<RDKit::ROMol*>&           mols,
  ...
  const int batchesPerGpu = hardwareOptions.batchesPerGpu == -1 ? omp_get_max_threads() : hardwareOptions.batchesPerGpu;
  const int numThreads =
    hardwareOptions.preprocessingThreads == -1 ? omp_get_max_threads() : hardwareOptions.preprocessingThreads;
  const int batchSize = hardwareOptions.batchSize;
...
#pragma omp          parallel for num_threads(numThreads) default(none) shared(sortedMols, eargs, paramsCopy)
  ...
  for (int i = 0; i < effectivebatchesPerGpu; ++i) {
...
#pragma omp parallel num_threads(effectivebatchesPerGpu) default(none) shared(streamsPerThread,     \
                                                                                devicesPerThread,   \
                                                                                sortedMols,         \
                                                                                eargs,              \
                                                                                paramsCopy,         \
