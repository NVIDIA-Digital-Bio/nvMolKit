...
void        initETKDGPipeline(std::vector<RDKit::ROMol*>&                 mols,
                              ...
                              DGeomHelpers::Dimensionality                dimensionality,
                              const int                                   confsPerMol,
                              const int                                   numThreads) {
  // Process only unique molecules, then duplicate.
  ...
  // by calling prepareEmbedderArgs on the first molecule before parallel processing
  if (numMols > 0) {
    EmbedArgs dummyEarg;
    nvMolKit::DGeomHelpers::prepareEmbedderArgs(*mols[0], params, dummyEarg, false);
  }

  // Step 1: Parallel initial setup for each molecule
#pragma omp parallel for num_threads(numThreads) default(none) shared(mols, eargs, params, dimensionality, numMols)
  for (int i = 0; i < numMols; i++) {
    ROMol* mol  = mols[i];
    auto&  earg = eargs[i];

    // Prepare initial embedder arguments (parallelizable setup)
    if (!nvMolKit::DGeomHelpers::prepareEmbedderArgs(*mol, params, earg, false)) {
      throw std::runtime_error("Failed to prepare ETKDG parameters for molecule");
    }

    // Determine dimensionality
    switch (dimensionality) {
      case DGeomHelpers::Dimensionality::DIM_4D:
        earg.dim = 4;
        break;
      case DGeomHelpers::Dimensionality::DIM_3D:
      default:
        throw std::runtime_error("Unsupported dimensionality for ETKDG: " +
                                 std::to_string(static_cast<int>(dimensionality)));
    }
    // Set up topology bounds (before first triangle smoothing)
    nvMolKit::DGeomHelpers::EmbeddingOps::setupTopologyBounds(mol, earg.mmat, params, earg.etkdgDetails);
  }

  // Step 2: Prepare BoundsMatPtr vector for efficient batch triangle smoothing
  std::vector<::DistGeom::BoundsMatPtr> matricesForBatch;
  matricesForBatch.reserve(numMols);
  for (int i = 0; i < numMols; ++i) {
    matricesForBatch.push_back(eargs[i].mmat);
  }

  // Step 3: Initialize needSmoothing vector - all molecules need smoothing initially
  std::vector<uint8_t> needSmoothing(numMols, 1);

  // Step 4: First batch triangle smoothing attempt
  const double tol = 0.0;
  nvMolKit::triangleSmoothBoundsBatch(matricesForBatch, &needSmoothing, tol);

  // Step 5: Parallel setup of relaxed bounds for molecules that still need smoothing
#pragma omp parallel for num_threads(numThreads) default(none) shared(mols, eargs, params, numMols, needSmoothing)
  for (int i = 0; i < numMols; i++) {
    if (needSmoothing[i] != 0U) {
      nvMolKit::DGeomHelpers::EmbeddingOps::setupRelaxedBounds(mols[i], eargs[i].mmat, params);
    }
  }

  // Step 6: Second batch triangle smoothing attempt
  nvMolKit::triangleSmoothBoundsBatch(matricesForBatch, &needSmoothing, tol);

  // Step 7: Handle ignored smoothing failures if parameter allows
  if (params.ignoreSmoothingFailures) {
#pragma omp parallel for num_threads(numThreads) default(none) shared(mols, eargs, params, numMols, needSmoothing)
    for (int i = 0; i < numMols; i++) {
      if (needSmoothing[i] != 0U) {
        nvMolKit::DGeomHelpers::EmbeddingOps::setupIgnoredSmoothingBounds(mols[i], eargs[i].mmat, params);
      }
    }
  } else {
    // Check if any molecules still need smoothing - if so, throw error
    for (const uint8_t needs : needSmoothing) {
      if (needs != 0U) {
        throw std::runtime_error("Could not triangle bounds smooth molecules in batch");
      }
    }
  }

  std::vector<ROMol*>    newMolPtrs;
  std::vector<EmbedArgs> newEargs;
  for (int molIdx = 0; molIdx < numMols; molIdx++) {
    const auto& mol = mols[molIdx];
    // Duplicate the molecule for each conformer
    for (int i = 0; i < confsPerMol; i++) {
      newMolPtrs.push_back(mol);
      newEargs.push_back(eargs[molIdx]);  // Use the original earg as a template
    }
  }
  std::swap(mols, newMolPtrs);
  std::swap(eargs, newEargs);
}
