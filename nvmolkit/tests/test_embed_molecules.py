...
def embed_with_rdkit(molecules, confs_per_mol=5):
    ...
    for mol in molecules:
        # Embed multiple conformers using RDKit
        conf_ids = rdDistGeom.EmbedMultipleConfs(mol, numConfs=confs_per_mol, randomSeed=42)
...
def test_embed_molecules_serial_vs_rdkit(embed_test_mols):
    ...
    using nvMolKit vs RDKit's EmbedMultipleConfs function.
    """
    confs_per_mol = 5
    ...
    # Get RDKit reference conformer counts
    rdkit_conf_ids = embed_with_rdkit(rdkit_mols, confs_per_mol)
    ...
    for mol in nvmolkit_mols:
        ...
        hardware_opts.numThreads = 1
        hardware_opts.batchSize = 1
        hardware_opts.batchesPerGpu = 1
...
def test_embed_molecules_batch_vs_rdkit(embed_test_mols, gpu_ids):
    ...
    in batch mode using nvMolKit vs individual RDKit embedding.
    """
    ...
    hardware_opts = embed.BatchHardwareOptions()
    hardware_opts.numThreads = 4
    hardware_opts.batchSize = 10
    hardware_opts.batchesPerGpu = 2
    hardware_opts.gpuIds = gpu_ids  # Use specified GPUs
...
def test_embed_molecules_with_hardware_options(embed_test_mols):
    """Test nvMolKit EmbedMolecules using BatchHardwareOptions struct."""
    confs_per_mol = 3
    ...
    # Create hardware options using the C++ struct
    hardware_opts = embed.BatchHardwareOptions()
    hardware_opts.numThreads = 2
    hardware_opts.batchSize = 5
    hardware_opts.batchesPerGpu = 1
