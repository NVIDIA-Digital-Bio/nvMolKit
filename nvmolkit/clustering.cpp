  ...
  boost::python::def(
    ...
    +[](const boost::python::dict& distanceMatrix,
      ...
      if (returnCentroids) {
        ...
        return boost::python::make_tuple(nvMolKit::toOwnedPyArray(clusterArray),
                                         nvMolKit::toOwnedPyArray(centroidArray));
      } else {
        nvMolKit::butinaGpu(matSpan, toSpan(clusterIds), cutoff, neighborlistMaxSize, {}, stream);
      }

      return nvMolKit::toOwnedPyArray(nvMolKit::makePyArray(clusterIds, boost::python::make_tuple(matDim1)));
    },
